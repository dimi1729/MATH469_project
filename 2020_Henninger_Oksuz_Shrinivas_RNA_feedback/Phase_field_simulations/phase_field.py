...
def return_noise(mesh, time_step, noise_strength):
    ...
    """
    p = fp.GaussianNoiseVariable(
        mesh=mesh, variance=noise_strength / (time_step * mesh.cellVolumes)
    )
    p = p - np.mean(p.value)
    return p
...
def run_CH(args):
    ...
    **Input variables**
    ...
    for par in par_values:
        ...
        if not fh:
            if changing_chi == 2:
                FE = f_en.free_energy_changing_chi(
                    c_alpha=c_alpha,
                    c_beta=c_beta,
                    rho_s=rho_s,
                    rho_r=rho_r,
                    ...
                    ratio=input_parameters["ratio"],
                    p=input_parameters["p"],
                )
            elif changing_chi == 1:
                FE = f_en.free_energy_changing_chi_LG(
                    c_alpha=c_alpha,
                    c_beta=c_beta,
                    rho_s=rho_s,
                    rho_r=rho_r,
                ...
                FE = f_en.free_energy(
                    c_alpha=c_alpha, c_beta=c_beta, rho_s=rho_s, chi=chi, kappa=kappa
                )

        else:
            if changing_chi == 2:
                FE = f_en.free_energy_FH_changing_chi(
                    c_alpha=c_alpha,
                    c_beta=c_beta,
                    rho_s=rho_s,
                    chi=chi,
                ...
                FE = f_en.free_energy_FH_changing_chi_LG(
                    c_alpha=c_alpha,
                    c_beta=c_beta,
                    rho_s=rho_s,
                    rho_r=rho_r,
        ...
        if "multiplier" in input_parameters.keys():
            rates = f_en.RNA_reactions(
                ...
                multiplier=input_parameters["multiplier"],
                m1=m1,
                kp_noise=kp_noise,
            )
        else:
        ...
        if dimension == 2:
            if int(input_parameters["circ_flag"]):
                mesh = sim_tools.create_circular_mesh(
                    radius=float(nx) * dx / 2, cellSize=dx * 1.5
                )
            else:
        ...
        phi_p = fp.CellVariable(
            mesh=mesh, name=r"$\phi_{prot}$", hasOld=True, value=phi_p_0
        )
        phi_r = fp.CellVariable(
            mesh=mesh, name=r"$\phi_{RNA}$", hasOld=True, value=phi_r_0
        )
        phi_p[:] = fp.GaussianNoiseVariable(
            mesh=mesh, mean=phi_p_0, variance=0.1 * phi_p_0
        ).value
        phi_p[phi_p < phi_p_0 * 0.9] = phi_p_0 * 0.9
        phi_p[phi_p > phi_p_0 * 1.1] = phi_p_0 * 1.1
        print(min(phi_p), max(phi_p), np.mean(phi_p))

        phi_r[:] = fp.GaussianNoiseVariable(
            mesh=mesh, mean=phi_r_0, variance=0.1 * phi_p_0
        ).value
        ...
        eqn0 = fp.TransientTerm(coeff=1.0, var=phi_p) == fp.DiffusionTerm(
            coeff=M_protein * FE.dmu_p_dphi_p(phi_p, phi_r), var=phi_p
        ) + fp.DiffusionTerm(
            coeff=M_protein * FE.dmu_p_dphi_r(phi_p, phi_r), var=phi_r
        ) - fp.DiffusionTerm(
            coeff=(M_protein, FE.kappa), var=phi_p
        ) + M_protein * return_noise(mesh, time_step, noise_strength)
        if not int(input_parameters["mu_r"]):
            eqn1 = fp.TransientTerm(coeff=1.0, var=phi_r) == fp.DiffusionTerm(
                coeff=M_rna, var=phi_r
            ) + rates.production(phi_p, phi_r, t) - rates.degradation(
                phi_p, phi_r
            ) + M_rna * return_noise(mesh, time_step, noise_strength)
        else:
            eqn1 = fp.TransientTerm(coeff=1.0, var=phi_r) == fp.DiffusionTerm(
                coeff=M_rna * FE.dmu_p_dphi_r(phi_p, phi_r), var=phi_p
            ) + fp.DiffusionTerm(
                coeff=M_rna * FE.dmu_r_dphi_r(phi_p, phi_r), var=phi_r
            ) + rates.production(phi_p, phi_r, t) - rates.degradation(
                phi_p, phi_r
            ) + M_rna * return_noise(mesh, time_step, noise_strength)
        ...
        while (elapsed <= duration) and (steps <= total_steps) and (dt > dt_min):
            ...
            if max(res0, res1) > tolerance:
                ...
                if steps % checkpoint == 0:
                    if changing_chi == 1:
                        fp.TSVViewer(
                            vars=[phi_p, phi_r, FE.chi_eff(phi_r, phi_p)]
                        ).plot(
                            filename=output_dir
                            + "Mesh/mesh_{step}.txt".format(step=steps)
                        )
                    elif changing_chi == 2:
                        fp.TSVViewer(vars=[phi_p, phi_r, FE.chi_eff(phi_r)]).plot(
                            filename=output_dir
                            + "Mesh/mesh_{step}.txt".format(step=steps)
                        )

                    else:
                        fp.TSVViewer(vars=[phi_p, phi_r]).plot(
                            filename=output_dir
                            + "Mesh/mesh_{step}.txt".format(step=steps)
                        )
                    ...
                    if (dimension == 2) and (plot_flag):
                        ...
                        fig.savefig(
                            fname=output_dir
                            + "Images/P_step_{step}.png".format(step=steps),
                            dpi=300,
                            format="png",
                        )
                        ...
                        if input_parameters["svg_flag"]:
                            ...
                            fig.savefig(
                                fname=output_dir
                                + "Images/P_step_{step}.svg".format(step=steps),
                                dpi=600,
                                format="svg",
                            )
                        plt.close()
                        ...
                        cs = ax.tricontourf(
                            ...
                            levels=np.linspace(
                                0,
                                2.5e-1
                                + 1.15 * k_production * c_beta / (k_degradation + 1e-9),
                                256,
                            ),
                        )
                        fig.colorbar(cs)
                        ...
                        fig.savefig(
                            fname=output_dir
                            + "Images/R_step_{step}.png".format(step=steps),
                            dpi=300,
                            format="png",
                        )
                        if input_parameters["svg_flag"]:
                            ...
                            fig.savefig(
                                fname=output_dir
                                + "Images/R_step_{step}.svg".format(step=steps),
                                dpi=600,
                                format="svg",
                            )
                        plt.close()
                        ...
                        if changing_chi:
                            ...
                            fig.savefig(
                                fname=output_dir
                                + "Images/chi_step_{step}.png".format(step=steps),
                                dpi=300,
                                format="png",
                            )
                            if input_parameters["svg_flag"]:
                                ...
                                fig.savefig(
                                    fname=output_dir
                                    + "Images/chi_step_{step}.svg".format(step=steps),
                                    dpi=600,
                                    format="svg",
                                )
                            plt.close()
                            ...
                            "\t".join(
                                [
                                    ...
                                    for it in [
                                        ...
                                        t.value,
                                        dt,
                                        sim_tools.get_radius(
                                            phi_p,
                                            mesh,
                                            dimension=dimension,
                                            threshold=0.5 * (c_alpha + c_beta),
                                        ),
                                        min(phi_p),
                                        max(phi_p),
                                        min(phi_r),
                                        max(phi_r),
                                        np.mean(phi_p * mesh.cellVolumes),
                                        np.mean(phi_r * mesh.cellVolumes),
                                        np.sum(
                                            (
                                                FE.f(phi_p, phi_r) * mesh.cellVolumes
                                            ).value
                                        ),
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Take output filename to run CH simulations"
    )
    ...
    parser.add_argument(
        "--pN", help="Parameter number from file (indexed from 1)", required=False
    )
