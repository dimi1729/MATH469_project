    ...
    **Input parameters**
    ...
    if image_list(dir_name, file_names, "c"):
        clip = mp.ImageSequenceClip(
            image_list(dir_name, file_names, "c"), fps=1 / duration
        )
...
def generate_images_3D(
    dir_name, label_idx=3, N=30, colormap="Blues", vmin=0.0, vmax=0.8, opacity=0.2
):
    """
    ...
    **Input parameters**
    ...
    iso = mlab.points3d(
        phi_a, opacity=opacity, vmin=vmin, vmax=vmax, colormap=colormap, line_width=0.2
    )
    mlab.outline()
    if label_idx == 3:
